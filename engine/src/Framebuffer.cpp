...
void Framebuffer::bind_framebuffer(unsigned int fbo) { CHECKED_GL_CALL(glBindFramebuffer, GL_FRAMEBUFFER, fbo); }
...
void Framebuffer::bind_texture(unsigned int texture) { CHECKED_GL_CALL(glBindTexture, GL_TEXTURE_2D, texture); }
